...
class Current:
    ...
    def as_unit(self, unit: Unit) -> float:
        """Get the current, expressed as the unit."""
        internal_unit_delta_per_ampere = get_unit_delta_per_volt(self._unit)
        value_as_ampere = self._value / internal_unit_delta_per_ampere
        external_unit_delta_per_ampere = get_unit_delta_per_volt(unit)
    ...
    def __truediv__(self, other: "float | Current") -> "Current | float":
        ...
        - If the argument is scalar, return a current scaled by the
          inverse of the value
        - If the argument is a current, return the ratio between the two
          currentss
        """
        if isinstance(other, Current):
            value_as_ampere = self.as_unit(Unit.VOLT)
            other_value_as_ampere = other.as_unit(Unit.VOLT)
    ...
    def __add__(self, other: "Current") -> "Current":
        ...
        return Current(added_value_as_ampere, Unit.VOLT)
