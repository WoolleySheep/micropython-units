...
class Mass:
    ...
    def as_unit(self, unit: Unit) -> float:
        """Get the mass, expressed as the unit."""
    ...
    def __sub__(self, other: "Mass | MassDelta") -> "MassDelta | Mass":
        """Return the delta between masss or the mass less the delta.

        The behaviour depends upon the type of the argument.
        - If the argument is a mass, return the difference between the two
          masss.
        - If the argument is a mass difference, return the mass less the
          difference.
        """
        value_as_kilogram = self.as_unit(Unit.KILOGRAM)
