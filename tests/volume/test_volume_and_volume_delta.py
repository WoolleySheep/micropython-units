...
class VolumeAndVolumeDeltaTest(unittest.TestCase):
    ...
    def test_add_volume_delta_to_volume_produces_volume(self) -> None:
        volume = Volume(0, VolumeUnit.CUBIC_METRE)
        ...
        self.assertAlmostEqual(1, new_volume_left_add.as_unit(VolumeUnit.CUBIC_METRE))
        self.assertIsInstance(new_volume_right_add, Volume)
        self.assertAlmostEqual(1, new_volume_right_add.as_unit(VolumeUnit.CUBIC_METRE))
    ...
    def test_subtract_volume_delta_from_volume_produces_volume(
        ...
        self.assertAlmostEqual(0, new_volume.as_unit(VolumeUnit.CUBIC_METRE))

    def test_subtract_volume_from_volume_produces_volume_delta(
        self,
    ) -> None:
        volume1 = Volume(3, VolumeUnit.CUBIC_METRE)
        volume2 = Volume(2, VolumeUnit.CUBIC_METRE)
        new_volume_delta = volume1 - volume2
        self.assertIsInstance(new_volume_delta, VolumeDelta)
        self.assertAlmostEqual(1, new_volume_delta.as_unit(VolumeUnit.CUBIC_METRE))
